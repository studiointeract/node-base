...
Usage:
  `basename $0` bucket srcfile targfile [acl] [mime_type]
...
# Inputs.
aws_ak=${AWS_S3_ACCESS_KEY?}                                             # access key
aws_sk=${AWS_S3_SECRET_ACCESS_KEY?}                                      # secret key
bucket=`printf $1 | awk 'BEGIN{FS="@"}{print $1}'`                       # bucket name
region=`printf $1 | awk 'BEGIN{FS="@"}{print ($2==""?"us-east-1":$2)}'`  # region name
srcfile="$2"                                                             # source file
targfile=`echo -n "$3" | sed "s/\/$/\/$(basename $srcfile)/"`            # target file
acl=${4:-'public-read'}                                                  # s3 perms
mime=${5:-"`guessmime "$srcfile"`"}                                      # mime type
